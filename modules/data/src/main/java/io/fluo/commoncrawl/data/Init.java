    ...
    JavaRDD<String> links = pages.flatMap(
        new FlatMapFunction<Page, String>() {
          ...
          public Iterable<String> call(Page page) throws Exception {
            ...
            if (links.size() > 0) {
              retval.add(String.format("p:%s\t%s:%s", pageUri, ColumnConstants.STATS, ColumnConstants.CRAWLS));
              retval.add(String.format("p:%s\t%s:%s", pageUri, ColumnConstants.STATS, ColumnConstants.PAGESCORE));
            ...
            for (Link link : links) {
              ...
              retval.add(String.format("p:%s\t%s:%s", pageUri, ColumnConstants.STATS, ColumnConstants.OUTLINKCOUNT));
              retval.add(String.format("p:%s\t%s:%s\t%s", pageUri, ColumnConstants.OUTLINKS, linkUri, link.getAnchorText()));
              retval.add(String.format("p:%s\t%s:%s", linkUri, ColumnConstants.STATS, ColumnConstants.INLINKCOUNT));
              retval.add(String.format("p:%s\t%s:%s", linkUri, ColumnConstants.STATS, ColumnConstants.PAGESCORE));
        ...
        new PairFlatMapFunction<Tuple2<String, Long>, String, Long>() {
          ...
          public Iterable<Tuple2<String, Long>> call(Tuple2<String, Long> t)
            ...
            if (args[0].startsWith("d:") && (args[1].startsWith(ColumnConstants.PAGES))) {
              ...
              retval.add(new Tuple2<>(String.format("%s\t%s:%s:%s", domain, ColumnConstants.PAGEDESC,
                                                    numLinksEnc, link), numLinks));
              retval.add(new Tuple2<>(String.format("%s\t%s:%s", domain, ColumnConstants.STATS,
